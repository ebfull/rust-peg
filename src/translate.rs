...
fn make_parse_state(ctxt: &rustast::ExtCtxt, rules: &[Rule]) -> Vec<rustast::P<rustast::Item>> {
	...
	let mut ctx_fields: Vec<rustast::TokenTree> = Vec::new();
	let mut ctx_init: Vec<rustast::TokenTree> = Vec::new();
	for rule in rules {
		if rule.context {
	...
	items.push(quote_item!(ctxt,
		impl ParseState {
			...
			fn mark_failure(&mut self, pos: usize, expected: String) -> RuleResult<()> {
...
pub fn header_items(ctxt: &rustast::ExtCtxt) -> Vec<rustast::P<rustast::Item>> {
	...
	items.push(quote_item!(ctxt,
		fn slice_eq(input: &str, state: &mut ParseState, pos: usize, m: String) -> RuleResult<()> {
			#![inline]
			#![allow(dead_code)]
	...
	items.push(quote_item!(ctxt,
		fn slice_eq_case_insensitive(input: &str, state: &mut ParseState, pos: usize, m: String) -> RuleResult<()> {
			#![inline]
			#![allow(dead_code)]
	...
	items.push(quote_item!(ctxt,
		fn any_char(input: &str, state: &mut ParseState, pos: usize) -> RuleResult<()> {
			...
			if input.len() > pos {
				...
				state.mark_failure(pos, "<character>".to_string())
			}
		}
	).unwrap());
...
fn compile_rule(ctxt: &rustast::ExtCtxt, grammar: &Grammar, rule: &Rule) -> rustast::P<rustast::Item> {
	...
	let wrapped_body = if cfg!(feature = "trace") {
		if rule.context {
			quote_expr!(ctxt, {
				...
				println!("[PEG_TRACE] Attempting to init context rule {} at {}:{} (pos {})", $rule_name, line, col, pos);
	...
	if rule.context {
		quote_item!(ctxt,
			fn $name<'input>(input: &'input str, pos: usize) -> RuleResult<$ret> {
				$wrapped_body
			}
		).unwrap()
	} else if rule.cached {
...
fn compile_state_and_then(ctxt: &rustast::ExtCtxt, name: &str, then: rustast::P<rustast::Expr>) -> rustast::P<rustast::Expr> {
	let ctx_name = rustast::str_to_ident(&format!("{}_ctx", name));
...
fn compile_expr(ctxt: &rustast::ExtCtxt, grammar: &Grammar, e: &Expr, result_used: bool) -> rustast::P<rustast::Expr> {
	match *e {
		...
		LiteralExpr(ref s, ref case_insensitive) => {
			let sl: &str = &s;
			if *case_insensitive {
				quote_expr!(ctxt, slice_eq_case_insensitive(input, state, pos, $sl.to_string()))
			}
			else {
				quote_expr!(ctxt, slice_eq(input, state, pos, $sl.to_string()))
			}
		}
		...
		CharSetExpr(invert, ref cases) => {
			...
			let (in_set, not_in_set) = cond_swap(invert, (
				...
				quote_expr!(ctxt, state.mark_failure(pos, $expected_str.to_string())),
			));
			...
			quote_expr!(ctxt, if input.len() > pos {
				...
				state.mark_failure(pos, $expected_str.to_string())
			})
		}
		...
		ActionExpr(ref exprs, ref code, is_cond) => {
			fn write_ctx(ctxt: &rustast::ExtCtxt, grammar: &Grammar, rules: &[Rule], exprs: &[TaggedExpr], code: &str, is_cond: bool) -> rustast::P<rustast::Expr> {
				match rules.first() {
					Some(ref first) => {
						let name = &first.name;

						if first.context {
							compile_state_and_then(ctxt, name,
								write_ctx(ctxt, grammar, rules.tail(), exprs, code, is_cond)
							)
						} else {
							write_ctx(ctxt, grammar, rules.tail(), exprs, code, is_cond)
						}
						...
						if is_cond {
							quote_expr!(ctxt, {
								...
								match $code_block {
									...
									Err(expected) => {
										state.mark_failure(pos, expected.to_string());
										Failed
									},
								}
							})
			...
			fn write_seq(ctxt: &rustast::ExtCtxt, grammar: &Grammar, rules: &[Rule], exprs: &[TaggedExpr], code: &str, is_cond: bool) -> rustast::P<rustast::Expr> {
				match exprs.first() {
					Some(ref first) => {
						let name = first.name.as_ref().map(|s| &s[..]);
						compile_match_and_then(ctxt, grammar, &*first.expr, name,
							write_seq(ctxt, grammar, rules, exprs.tail(), code, is_cond)
						)
					}
					None => {
			...
			let body = write_seq(ctxt, grammar, &grammar.rules, &exprs, &code, is_cond);
			quote_expr!(ctxt, {
				let start_pos = pos;
				$body
			})
