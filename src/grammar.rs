        ...
        let choice_res =
            {
                let start_pos = pos;
                {
                    let seq_res =
                        {
                            let choice_res =
                                slice_eq(input, state, pos, "#[export]".to_string());
                            match choice_res {
                                Matched(pos, value) => Matched(pos, value),
                                Failed =>
                                slice_eq(input, state, pos, "#[pub]".to_string()),
                            }
                        };
                    match seq_res {
                        Matched(pos, _) => {
        ...
        let choice_res =
            {
                let start_pos = pos;
                {
                    let seq_res = slice_eq(input, state, pos, "#[cache]".to_string());
                    match seq_res {
                        Matched(pos, _) => {
                            {
                                let seq_res = parse___(input, state, pos);
...
fn parse_contextflag<'input>(input: &'input str, state: &mut ParseState,
                             pos: usize) -> RuleResult<bool> {
    {
        let choice_res =
            {
                let start_pos = pos;
                {
                    let seq_res = slice_eq(input, state, pos, "#[context]".to_string());
                    match seq_res {
                        Matched(pos, _) => {
                            {
                                let seq_res = parse___(input, state, pos);
...
fn parse_rust_use<'input>(input: &'input str, state: &mut ParseState,
                          pos: usize) -> RuleResult<RustUse> {
    {
        let start_pos = pos;
        {
            let seq_res = slice_eq(input, state, pos, "use".to_string());
            match seq_res {
                Matched(pos, _) => {
                    {
                        let seq_res = parse___(input, state, pos);
        ...
        let start_pos = pos;
        {
            ...
            match seq_res {
                Matched(pos, _) => {
                    {
                        let seq_res =
                            match slice_eq(input, state, pos, "?".to_string()) {
                                Matched(newpos, value) => {
...
fn parse_nonBraceCharacter<'input>(input: &'input str, state: &mut ParseState,
    ...
    if input.len() > pos {
        ...
        match ch {
            '{' | '}' => state.mark_failure(pos, "[^{}]".to_string()),
            _ => Matched(next, ()),
        }
    } else { state.mark_failure(pos, "[^{}]".to_string()) }
}
                        ...
                        let seq_res =
                            {
                                ...
                                match choice_res {
                                    ...
                                    Matched(pos, value),
                                    Failed =>
                                    slice_eq(input, state, pos, "_".to_string()),
                                }
                            };
                        match seq_res {
                            Matched(pos, _) => {
        ...
        let start_pos = pos;
        {
            ...
            match seq_res {
                Matched(pos, value) => {
                    {
                        let seq_res =
                            match slice_eq(input, state, pos, "i".to_string()) {
                                Matched(newpos, value) => {
                                    Matched(newpos, Some(value))
                                }
                                Failed => { Matched(pos, None) }
            ...
            let seq_res =
                {
                    let assert_res =
                        {
                            let choice_res = slice_eq(input, state, pos, "]".to_string());
                            match choice_res {
                                Matched(pos, value) => Matched(pos, value),
                                Failed => {
                                    let choice_res =
 ...
 -> RuleResult<char> {
    {
        let start_pos = pos;
        {
            let seq_res = slice_eq(input, state, pos, "\\0".to_string());
            match seq_res {
                Matched(pos, _) => {
                    {
                        let seq_res =
 ...
 -> RuleResult<char> {
    {
        let start_pos = pos;
        {
            let seq_res = slice_eq(input, state, pos, "\\x".to_string());
            match seq_res {
                Matched(pos, _) => {
                    {
                        let seq_res =
 ...
 -> RuleResult<char> {
    {
        let start_pos = pos;
        {
            let seq_res = slice_eq(input, state, pos, "\\u{".to_string());
            match seq_res {
                Matched(pos, _) => {
                    {
                        let seq_res =
...
fn parse_digit<'input>(input: &'input str, state: &mut ParseState, pos: usize)
 -> RuleResult<()> {
    if input.len() > pos {
        ...
        match ch {
            '0' ...'9' => Matched(next, ()),
            _ => state.mark_failure(pos, "[0-9]".to_string()),
        }
    } else { state.mark_failure(pos, "[0-9]".to_string()) }
}
fn parse_hexDigit<'input>(input: &'input str, state: &mut ParseState,
    ...
    if input.len() > pos {
        ...
        match ch {
            ...
            _ => state.mark_failure(pos, "[0-9a-fA-F]".to_string()),
        }
    } else { state.mark_failure(pos, "[0-9a-fA-F]".to_string()) }
}
...
fn parse_lowerCaseLetter<'input>(input: &'input str, state: &mut ParseState,
    ...
    if input.len() > pos {
        ...
        match ch {
            'a' ...'z' => Matched(next, ()),
            _ => state.mark_failure(pos, "[a-z]".to_string()),
        }
    } else { state.mark_failure(pos, "[a-z]".to_string()) }
}
        ...
        match choice_res {
            ...
            Failed => {
                ...
                match choice_res {
                    Matched(pos, value) => Matched(pos, value),
                    Failed => {
                        let choice_res = slice_eq(input, state, pos, "\r".to_string());
                        match choice_res {
                            Matched(pos, value) => Matched(pos, value),
                            Failed => {
                                let choice_res =
                                    slice_eq(input, state, pos, "\u{2028}".to_string());
                                match choice_res {
                                    Matched(pos, value) =>
                                    Matched(pos, value),
                                    Failed =>
                                    slice_eq(input, state, pos, "\u{2029}".to_string()),
                                }
                            }
                        }
                    }
